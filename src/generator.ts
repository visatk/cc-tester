...
export function getTestCard(
	...
	if (!card)
		throw new Error(
			`[cc-tester] Test scenario '${scenario}' is not currently supported for network '${network}'.`,
		);
	return card;
}
...
export function generateRandomCard(input = "random"): CardInfo {
	...
	if (!prefix)
		throw new Error("[cc-tester] Invalid prefix or network provided.");
	...
	const expMonth = ((parseInt(getSecureRandomDigit(), 10) % 12) + 1)
		.toString()
		.padStart(2, "0");
	const expYear = (
		currentYear +
		(parseInt(getSecureRandomDigit(), 10) % 5) +
		1
	).toString();
