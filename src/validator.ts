...
export function validateCardStrict(
	...
	if (isNetworkSupported) {
		...
		if (network === "amex" && len === 15) isLengthValid = true;
			...
			isLengthValid = true;
		else if (network === "unionpay" || network === "maestro")
			isLengthValid = len >= 12 && len <= 19;

		if (!isLengthValid)
			errors.push(`Invalid length (${len}) for network ${network}.`);
	}
	...
	if (expMonth && expYear) {
		...
		if (month < 1 || month > 12) {
			...
			(year === currentYearShort && month < currentMonth)
		) {
			isNotExpired = false;
	...
	const isValid =
		isLuhnValid && isNetworkSupported && isLengthValid && isNotExpired;
